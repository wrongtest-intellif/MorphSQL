...
statement
    ...
    | CREATE sql_namespace (IF NOT EXISTS)? multipartIdentifier
        (commentSpec |
         locationSpec |
         (WITH (DBPROPERTIES | PROPERTIES) tablePropertyList))*        #createNamespace
    | ALTER sql_namespace multipartIdentifier
        SET (DBPROPERTIES | PROPERTIES) tablePropertyList              #setNamespaceProperties
    | ALTER sql_namespace multipartIdentifier
        SET locationSpec                                               #setNamespaceLocation
    | DROP sql_namespace (IF EXISTS)? multipartIdentifier
    ...
    | (DESC | DESCRIBE) sql_namespace EXTENDED?
        multipartIdentifier                                            #describeNamespace
    | (DESC | DESCRIBE) TABLE? option=(EXTENDED | FORMATTED)?
        multipartIdentifier partitionSpec? describeColName?            #describeRelation
    | (DESC | DESCRIBE) QUERY? query                                   #describeQuery
    | COMMENT ON sql_namespace multipartIdentifier IS
        comment=(STRING | NULLVAL)                                        #commentNamespace
    | COMMENT ON TABLE multipartIdentifier IS comment=(STRING | NULLVAL)  #commentTable
...
queryTerm
    ...
    | left=queryTerm {legacy_setops_precedence_enabled}?
        queryOperator=(INTERSECT | UNION | EXCEPT | SETMINUS) setQuantifier? right=queryTerm  #setOperation
    | left=queryTerm {!legacy_setops_precedence_enabled}?
        queryOperator=INTERSECT setQuantifier? right=queryTerm                                #setOperation
    | left=queryTerm {!legacy_setops_precedence_enabled}?
        queryOperator=(UNION | EXCEPT | SETMINUS) setQuantifier? right=queryTerm              #setOperation
    ;
...
sortItem
    : expression ordering=(ASC | DESC)? (NULLVALS nullOrder=(LAST | FIRST))?
    ;
...
rowFormat
    ...
    | ROW FORMAT DELIMITED
      ...
      (NULLVAL DEFINED AS nullDefinedAs=STRING)?                                       #rowFormatDelimited
    ;
...
booleanExpression
    ...
    | left=booleanExpression queryOperator=AND right=booleanExpression  #logicalBinary
    | left=booleanExpression queryOperator=OR right=booleanExpression   #logicalBinary
    ;
...
valueExpression
    ...
    | queryOperator=(MINUS | PLUS | TILDE) valueExpression                                        #arithmeticUnary
    | left=valueExpression queryOperator=(ASTERISK | SLASH | PERCENT | DIV) right=valueExpression #arithmeticBinary
    | left=valueExpression queryOperator=(PLUS | MINUS | CONCAT_PIPE) right=valueExpression       #arithmeticBinary
    | left=valueExpression queryOperator=AMPERSAND right=valueExpression                          #arithmeticBinary
    | left=valueExpression queryOperator=HAT right=valueExpression                                #arithmeticBinary
    | left=valueExpression queryOperator=PIPE right=valueExpression                               #arithmeticBinary
...
primaryExpression
    ...
    | FIRST '(' expression (IGNORE NULLVALS)? ')'                                                 #first
    | LAST '(' expression (IGNORE NULLVALS)? ')'                                                  #last
    ...
    | functionName '(' (setQuantifier? argument+=expression (',' argument+=expression)*)? ')'
       ...
       (nullsOption=(IGNORE | RESPECT) NULLVALS)? ( OVER windowSpec)?                             #functionCall
...
qualifiedColTypeWithPosition
    : name=multipartIdentifier dataType (NOT NULLVAL)? commentSpec? colPosition?
    ;
...
colType
    : colName=errorCapturingIdentifier dataType (NOT NULLVAL)? commentSpec?
    ;
...
complexColType
    : identifier ':' dataType (NOT NULLVAL)? commentSpec?
    ;
...
alterColumnAction
    ...
    | commentSpec
    | colPosition
    | setOrDrop=(SET | DROP) NOT NULLVAL
    ;
